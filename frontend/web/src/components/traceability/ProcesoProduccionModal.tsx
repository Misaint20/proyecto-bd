...
import type { ProcesoProduccion, Lote } from "@/types/traceability"

interface ProcesoProduccionModalProps {
    isOpen: boolean
    onClose: () => void
    onSuccess: () => void
    proceso?: ProcesoProduccion | null
}
...
export default function ProcesoProduccionModal({ isOpen, onClose, onSuccess, proceso }: ProcesoProduccionModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const procesoId = (proceso as any)?.id_proceso ?? proceso?.id_proceso
        const isUpdate = !!procesoId

        const dataToSend = {
            ...formData,
        }

        const result = isUpdate ? await updateProcesoProduccion(procesoId as string, dataToSend) : await createProcesoProduccion(dataToSend)
    ...
    return (
        <BaseModal
            isOpen={isOpen}
            onClose={onClose}
            title={proceso ? "Editar Proceso" : "Nuevo Proceso"}
            subtitle={proceso ? "Actualiza los datos del proceso" : "Registra un nuevo proceso de producción"}
